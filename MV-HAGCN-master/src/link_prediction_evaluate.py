...
def predict_model(model, file_name, feature, A, B, o_ass, eval_type, lr, length, layer, dataset):
    ...
    aucs, f1s, recalls, accs, fprs, tprs, auprs, ps_list, rs_list = [], [], [], [], [], [], [], [], [],
    losss = []
    epoch = args.epoch
    ...
    for _ in range(1):
        for iter_ in range(epoch):
            ...
            for i in range(edge_type_count):
                if eval_type == 'all':
                    train_auc, triain_f1, train_recall, train_acc, fpr0, tpr0, aupr0, ps, rs = link_prediction_evaluate(
                        final_model,
                    ...
                    test_auc, test_f1, test_recall, test_acc, fpr, tpr, aupr1, ps, rs = link_prediction_evaluate(final_model,
    ...
    return aucs[max_iter], f1s[max_iter], recalls[max_iter], accs[max_iter], fprs[max_iter], tprs[max_iter], auprs[
        max_iter], ps_list[max_iter], rs_list[max_iter]